...
def read_csv_files_in_folder(folder: str, algorithm: str, nodes: set, dps: set, remaining_nodes: dict, remaining_dps: dict, remaining_k: dict, original_k: dict, reduction_rule_names: list, applicability_ticks: dict, modifying_ticks: dict, throughKnownSolution: bool, ks: list) -> list:
    first_file = True
    titles = []
    for file in os.listdir(folder):
        ...
        with open(os.path.join(folder, file)) as csv_file:
            ...
            titles.append(title)
    return titles[::len(dps)]

def draw_kernel_size_plots(titles: list, nodes: set, dps: set, remaining_nodes: dict, remaining_dps: dict) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    for titleIndex, node in enumerate(nodes):
        fig, ax = plot.subplots(figsize=(10, 6))
        ax.set_title("Remaining number of nodes and demand pairs in the kernel for a " + str(titles[titleIndex]), wrap=True)
...
def draw_kernel_size_plots_through_known_solution(titles: list, nodes: set, dps: set, remaining_nodes: dict, remaining_dps: dict, different_ks: int) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    ...
    for k in range(different_ks):
        for titleIndex, node in enumerate(nodes):
            fig, ax = plot.subplots(figsize=(10, 6))
            ax.set_title("Remaining number of nodes and demand pairs in the kernel for a " + str(titles[k * len(nodes) + titleIndex]), wrap=True)
            ax.set_xticklabels(dps)
            node_data = []
            dp_data = []
            cols = []
...
def draw_time_plots_applicability(titles: list, nodes: set, dps: set, reduction_rules_names: list, applicability_ticks: dict) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    number_of_reduction_rules = len(reduction_rules_names)
    for titleIndex, node in enumerate(nodes):
        fig, ax = plot.subplots(figsize=(22, 10))
        ax.set_title("Number of operations per reduction rule checking applicability for a " + str(titles[titleIndex]), wrap=True)
...
def draw_time_plots_applicability_through_known_solution(titles: list, nodes: set, dps: set, reduction_rules_names: list, applicability_ticks: dict, different_ks: int) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    number_of_reduction_rules = len(reduction_rules_names)
    ...
    for k in range(different_ks):
        for titleIndex, node in enumerate(nodes):
            fig, ax = plot.subplots(figsize=(22, 10))
            ax.set_title("Number of operations per reduction rule checking applicability for a " + str(titles[k * len(nodes) + titleIndex]), wrap=True)
            plot.ticklabel_format(axis="y", style="plain")
            ax.set_xticklabels(dps)
            applicability_ticks_data = []
            cols = []
...
def draw_time_plots_modifying(titles: list, nodes: set, dps: set, reduction_rules_names: list, modifying_ticks: dict) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    number_of_reduction_rules = len(reduction_rules_names)
    for titleIndex, node in enumerate(nodes):
        fig, ax = plot.subplots(figsize=(22, 10))
        ax.set_title("Number of operations per reduction rule to modify the instance for a " + str(titles[titleIndex]), wrap=True)
        plot.ticklabel_format(axis="y", style="plain")
        ax.set_xticklabels(dps)
...
def draw_time_plots_modifying_through_known_solution(titles: list, nodes: set, dps: set, reduction_rules_names: list, modifying_ticks: dict, different_ks: int) -> None:
    nodes = sorted(nodes)
    dps = sorted(dps)
    number_of_reduction_rules = len(reduction_rules_names)
    ...
    for k in range(different_ks):
        for titleIndex, node in enumerate(nodes):
            fig, ax = plot.subplots(figsize=(22, 10))
            ax.set_title("Number of operations per reduction rule to modify the instance for a " + str(titles[k * len(nodes) + titleIndex]), wrap=True)
            plot.ticklabel_format(axis="y", style="plain")
            ax.set_xticklabels(dps)
            modifying_ticks_data = []
            cols = []
...
def make_plots(folder: str, algorithm: str, throughKnownSolution: bool) -> None:
    ...
    applicability_ticks = {}
    modifying_ticks = {}
    titles = read_csv_files_in_folder(folder, algorithm, nodes, dps, remaining_nodes, remaining_dps, remaining_k, original_k, reduction_rules_names, applicability_ticks, modifying_ticks, throughKnownSolution, [2, 4, 6, 8, 10])
    if throughKnownSolution:
        draw_kernel_size_plots_through_known_solution(titles, nodes, dps, remaining_nodes, remaining_dps, 5)
        #draw_time_plots_applicability_through_known_solution(titles, nodes, dps, reduction_rules_names, applicability_ticks, 5)
        #draw_time_plots_modifying_through_known_solution(titles, nodes, dps, reduction_rules_names, modifying_ticks, 5)
    else:
        draw_kernel_size_plots(titles, nodes, dps, remaining_nodes, remaining_dps)
        #draw_time_plots_applicability(titles, nodes, dps, reduction_rules_names, applicability_ticks)
        #draw_time_plots_modifying(titles, nodes, dps, reduction_rules_names, modifying_ticks)

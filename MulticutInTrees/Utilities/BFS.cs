...
namespace MulticutInTrees.Utilities
{
    ...
    public static class BFS
    {
        ...
        /// <param name="graph">The <typeparamref name="TGraph"/> in which we are finding the shortest path.</param>
        ...
        public static List<TEdge> FindShortestPath<TGraph, TEdge, TNode>(TGraph graph, TNode startNode, HashSet<TNode> targetSet, Counter treeCounter, HashSet<TNode> seen = null) where TGraph : AbstractGraph<TEdge, TNode> where TEdge : Edge<TNode> where TNode : AbstractNode<TNode>
        {
#if !EXPERIMENT
            ...
            Dictionary<TNode, (TEdge, TNode)> pushingNode = new()
            {
            ...
            while (queue.Count > 0)
            {
                ...
                foreach (TEdge neighbouringEdge in graph.GetNeighbouringEdges(node, treeCounter))
                {
                    ...
                    if (targetSet.Contains(neighbour))
                    {
                        ...
                        {
                            path.Add(pushingNode[node].Item1);
                            node = pushingNode[node].Item2;
                        }
                        path.Reverse();
                        return path;
                    }
                    ...
                    pushingNode[neighbour] = (neighbouringEdge, node);
                }
            }

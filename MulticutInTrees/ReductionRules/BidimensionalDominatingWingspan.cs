...
namespace MulticutInTrees.ReductionRules
{
    ...
    public class BidimensionalDominatingWingspan : ReductionRule
    {
        ...
        private (Node, Node) ComputeWingspanEndpoints(Node node)
        {
            ...
            if (!DemandPairsPerNode.TryGetValue(node, out CountedCollection<DemandPair> pairsPerNode, Measurements.DemandPairsPerEdgeKeysCounter))
            {
                return (parent, parent);
            }

            foreach (DemandPair dp in pairsPerNode.GetCountedEnumerable(Measurements.DemandPairsPerEdgeValuesCounter))
            {

...
namespace TESTS_MulticutInTrees.ReductionRules
{
    ...
    public class UnitTestImprovedDominatedEdge
    {
        ...
        public void TestAfterDemandPathRemoved()
        {
            ...
            dominatedEdge = new ImprovedDominatedEdge(tree, demandPairs, new GuoNiedermeierKernelisation(instance), demandPairPerEdge);
        ...
        public void TestPathOfLength4()
        {
            ...
            DemandPair demandPair = new DemandPair(node3, node0);
            CountedList<DemandPair> demandPairs = new CountedList<DemandPair>(new List<DemandPair>() { demandPair }, counter);

            CountedDictionary<(TreeNode, TreeNode), CountedCollection<DemandPair>> demandPairPerEdge = new CountedDictionary<(TreeNode, TreeNode), CountedCollection<DemandPair>>(new Dictionary<(TreeNode, TreeNode), CountedCollection<DemandPair>>()
            {
                {(node0, node1), new CountedCollection<DemandPair>(new List<DemandPair>(){ demandPair }, counter) },
                {(node1, node2), new CountedCollection<DemandPair>(new List<DemandPair>(){ demandPair }, counter) },
                {(node2, node3), new CountedCollection<DemandPair>(new List<DemandPair>(){ demandPair }, counter) }
            }, counter);

            MulticutInstance instance = new MulticutInstance(InputTreeType.Fixed, InputDemandPairsType.Fixed, -1, tree, demandPairs, 100, 1);
            ImprovedDominatedEdge dominatedEdge = new ImprovedDominatedEdge(tree, demandPairs, new GuoNiedermeierKernelisation(instance), demandPairPerEdge);
            Assert.IsTrue(dominatedEdge.RunFirstIteration());
            Assert.AreEqual(2, tree.NumberOfNodes(counter));
        }
    }
}
